"""Performance tests and edge cases for context management."""
...
@pytest.mark.skip(reason="Performance tests are flaky and depend on specific timing/configuration")
class TestContextManagementPerformance:
    ...
    @pytest.fixture
    def context_manager(self, performance_config):
        """Context manager for performance testing."""
        mock_llm = Mock()
        return ContextManager(llm=mock_llm, config=performance_config)

    def test_large_conversation_processing_time(self, context_manager):
        """Test processing time for large conversations."""
        # Create a large conversation
        messages = []
        for i in range(500):  # 500 message pairs = 1000 messages
            messages.extend(
                [
                    HumanMessage(content=f"User question {i} with some additional context and details"),
                    AIMessage(content=f"Assistant response {i} with detailed explanation and analysis"),
                ]
            )
        ...
        with patch(
            "openchatbi.context_manager.call_llm_chat_model_with_retry",
            return_value=AIMessage(content="Conversation summary"),
        ):
        ...
        # Should significantly reduce message count (at least 80% reduction)
        assert (
            len(result) < len(messages) * 0.2
        ), f"Should reduce messages significantly: {len(result)} vs {len(messages)}"

    def test_large_tool_output_trimming_performance(self, context_manager):
        """Test performance of trimming very large tool outputs."""
        # Create very large tool outputs
        large_outputs = [
            "A" * 100000,  # 100KB
            "B" * 50000,  # 50KB
            json_like_output := '{"data": ['
            + ",".join([f'{{"id": {i}, "value": "data_{i}"}}' for i in range(10000)])
            + "]}",  # Large JSON
            csv_like_output := "\n".join(
                [f"col1,col2,col3\n"] + [f"val{i},data{i},info{i}" for i in range(5000)]
            ),  # Large CSV
        ]

        messages = []
        for i, output in enumerate(large_outputs):
            messages.append(ToolMessage(content=output, tool_call_id=f"tool_{i}"))
        ...
        # Should complete quickly even with large outputs
        assert processing_time < 0.5, f"Trimming took {processing_time:.2f}s, expected < 0.5s"

        # All outputs should be trimmed
        for msg in result:
            if isinstance(msg, ToolMessage):
                assert len(str(msg.content)) < 50000, "Tool output should be trimmed"

    def test_token_estimation_performance(self, context_manager):
        """Test performance of token estimation for large texts."""
        # Create various large texts
        test_texts = [
            "Short text",
            "Medium length text " * 100,
            "Very long repeated text " * 1000,
            "A" * 100000,  # Very long single character
            "Mixed content with numbers 123 symbols !@# and unicode 中文 " * 1000,
        ]
        ...
        assert processing_time < 0.1, f"Token estimation took {processing_time:.2f}s, expected < 0.1s"

    def test_concurrent_context_management(self, performance_config):
        """Test context management under concurrent-like conditions."""
        import threading
        ...
        def process_messages(thread_id):
            try:
                mock_llm = Mock()
                cm = ContextManager(llm=mock_llm, config=performance_config)

                messages = [HumanMessage(content=f"Thread {thread_id} message {i}") for i in range(100)]

                with patch(
                    "openchatbi.context_manager.call_llm_chat_model_with_retry",
                    return_value=AIMessage(content=f"Summary for thread {thread_id}"),
                ):
                    cm.manage_context_messages(messages)
                    result = messages
    ...
    def test_memory_usage_large_datasets(self, context_manager):
        ...
        for size_multiplier in [10, 50, 100, 200]:
            messages = []
            for i in range(size_multiplier):
                messages.extend(
                    [
                        HumanMessage(content=f"Question {i}" * 10),
                        AIMessage(content=f"Response {i}" * 20),
                        ToolMessage(content="X" * 1000, tool_call_id=f"tool_{i}"),
                    ]
                )
        ...
        assert object_growth < initial_objects * 10, f"Excessive memory growth: {object_growth} objects"
...
@pytest.mark.skip(reason="Stress tests require complex setup and may be flaky")
class TestContextManagementStressTests:
    """Stress tests for context management under extreme conditions."""

    def test_rapid_successive_calls(self):
        """Test rapid successive context management calls."""
        ...
        results = []
        for i in range(100):
            test_messages = messages + [HumanMessage(content=f"Call {i}")]
            context_manager.manage_context_messages(test_messages)
        ...
        assert total_time < 2.0, f"100 calls took {total_time:.2f}s, expected < 2.0s"
        assert len(results) == 100, "Should complete all calls"

    def test_maximum_message_count_handling(self, context_manager):
        """Test handling of maximum possible message counts."""
        # Create maximum reasonable number of messages
        max_messages = 10000
        messages = []
        ...
        with patch(
            "openchatbi.context_manager.call_llm_chat_model_with_retry",
            return_value=AIMessage(content="Large conversation summary"),
        ):
            context_manager.manage_context_messages(messages)
            result = messages
        ...
        assert processing_time < 5.0, f"Processing {max_messages} messages took {processing_time:.2f}s"
        assert len(result) < max_messages, "Should reduce message count"
