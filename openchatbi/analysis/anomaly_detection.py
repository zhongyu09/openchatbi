...
def _evaluate_window(
    actual_values: List[float], predicted_values: List[float], historical_values: List[float]
...
def evaluate_anomalies(
    ...
    if not input_data or len(input_data) <= evaluation_window:
        return 0.0, {
            "error": f"Input data length ({len(input_data)}) must be greater than evaluation window ({evaluation_window})."
        }

    # Check service availability
    if not _check_service_health(service_url):
        return 0.0, {
            "error": "Time Series Forecasting Service Unavailable. Cannot perform anomaly detection without the forecasting service."
        }
    ...
    if not predicted_values or len(predicted_values) != len(eval_values):
        return 0.0, {
            "error": f"Mismatch between predicted values ({len(predicted_values)}) and evaluation window size ({len(eval_values)})."
        }

    # Evaluate the window
    return _evaluate_window(actual_values=eval_values, predicted_values=predicted_values, historical_values=hist_values)
