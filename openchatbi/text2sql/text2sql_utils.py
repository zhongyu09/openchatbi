...
def init_sql_example_retriever(catalog):
    ...
    retriever = Chroma.from_texts(texts, embedding_model, collection_name="text2sql").as_retriever(
...
def init_table_selection_example_dict(catalog):
    ...
    retriever = Chroma.from_texts(texts, embedding_model, collection_name="table_selection_example").as_retriever(
        search_type="mmr", search_kwargs={"distance_metric": "cosine", "fetch_k": 30, "k": 10}
    )
