...
_TRAJECTORIES: dict[str, list[str | None]] = {
    ...
    "List ten customers with their id and name.": ["text2sql", None],
    "Show orders whose status is Shipped, including their id, customer, status and placed date.": ["text2sql", None],
    "How many orders have the status Shipped?": ["text2sql", None],
    "How many orders are there for each order status? Sort by the count, highest first.": ["text2sql", None],
    "For shipped orders, show the order id, status and the customer name.": ["text2sql", None],
    "List shipments with their tracking number, the related order id and the customer name.": ["text2sql", None],
    "Which customers have placed an order that is On Road?": ["text2sql", None],
    "Who are the top 5 customers by number of orders? Show their name and order count.": ["text2sql", None],
    "What are the distinct order item statuses used in the system?": ["text2sql", None],
    "Which customers have placed more than 10 orders? Show the customer id and their order count.": ["text2sql", None],
    "How many orders were placed in Q1 2024?": ["text2sql", None],
    "Compare this month's order volume to last month.": ["text2sql", None],
    "Which products appear in orders but have no shipment item?": ["text2sql", None],
    "Detect anomalies in daily order counts.": ["text2sql", "run_python_code", None],
    "Plot the monthly shipment trend.": ["text2sql", "run_python_code", None],
    "Forecast next quarter order volume from historical monthly orders.": ["text2sql", "run_python_code", None],
    "Generate a Q1 2024 order status report and save it.": ["text2sql", "save_report", None],
    "Generate a fulfillment report with shipment counts by order status and save it.": [
        "search_knowledge",
        "text2sql",
        "save_report",
        None,
    ],
    "What does product_name represent in this example schema?": ["search_knowledge", None],
    "Explain the Shipments table schema.": ["show_schema", None],
    "Find tables related to shipment tracking numbers and shipment dates.": ["search_schema", None],
    "How many distinct customers placed orders in the last 7 days?": ["text2sql", None],
    "Count orders by product category using the product_name prefix before a dash.": ["text2sql", None],
    "What share of all orders are Shipped?": ["text2sql", None],
    "What is the average number of items per order for each customer?": ["text2sql", None],
    "Show daily order volume for the last 30 days.": ["text2sql", None],
    "How many shipments were sent between 2024-01-01 and 2024-03-31?": ["text2sql", None],
    "Plot the monthly order trend for 2024.": ["text2sql", "run_python_code", None],
    "Generate a Q1 2024 product category order report using the product_name category rule and save it.": [
        "search_knowledge",
        "text2sql",
        "save_report",
        None,
    ],
    "Summarize daily order volume and save it as a report.": ["text2sql", "save_report", None],
    "Explain the Orders table schema.": ["show_schema", None],
    "What does order_status mean in this example schema?": ["search_knowledge", None],
}
